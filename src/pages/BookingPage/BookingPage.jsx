...
const CarromBoardBooking = ({ tableData , bookingCallBack}) => {
  ...
  useEffect(() => {
    const dataToUpdate = scheduleTable && tableData && scheduleTable.find(item => item.id == tableData.id);
  ...
  const tableDataExits = (userName) => {
    let quotaCount = 0;
        if (myTableData.a1_userId === userName || myTableData.a2_userId === userName ||
          myTableData.b1_userId === userName || myTableData.b2_userId === userName) {
            quotaCount++;
        };
  ...
  const countQuotaLimit = (userName) => {
    ...
    scheduleTable.forEach(slot => {
        if (slot.a1_userId === userName || slot.a2_userId === userName ||
            slot.b1_userId === userName || slot.b2_userId === userName) {
            quotaCount++;
        }
    });
  ...
  const bookSeat = (seat, name) => {
    ...
    const mySeat = seat == "A1" ? "a1_userId" : seat == "A2" ? "a2_userId" : seat == "B1" ? "b1_userId" : "b2_userId";
    const nameOfUser =  getUserInfo && getUserInfo.username === name ? "" : getUserInfo.username;
        ...
        <div className="flex justify-center">
          <div className="grid grid-cols-3 grid-rows-3 gap-4">
            <div className="col-start-2 row-start-1">
              <Seat getUserInfo={getUserInfo} seat="A1" name={myTableData && myTableData.a1_userId} disabled={myTableData && myTableData.a1_userId.length > 0} bookSeat={bookSeat} />
            </div>
            <div className="col-start-1 row-start-2">
              <Seat getUserInfo={getUserInfo} seat="B1" name={myTableData && myTableData.b1_userId} disabled={myTableData && myTableData.b1_userId.length > 0} bookSeat={bookSeat} />
            </div>
            <div className="col-start-3 row-start-2">
              <Seat getUserInfo={getUserInfo} seat="B2" name={myTableData && myTableData.b2_userId} disabled={myTableData && myTableData.b2_userId.length > 0} bookSeat={bookSeat} />
            </div>
            <div className="col-start-2 row-start-3">
              <Seat getUserInfo={getUserInfo} seat="A2" name={myTableData && myTableData.a2_userId} disabled={myTableData && myTableData.a2_userId.length > 0} bookSeat={bookSeat} />
            </div>
...
const Seat = ({ seat, name, disabled, bookSeat, getUserInfo }) => (
  getUserInfo && getUserInfo.username === name ?
    <div
      className={`p-2 border border-gray-400 text-center cursor-pointer ${disabled ? "bg-green-500 text-gray-600" : "hover:bg-green-300 transition duration-300"
        }`}
    ...
    <div
      className={`p-2 border border-gray-400 text-center cursor-pointer ${disabled ? "bg-gray-400 text-gray-600" : "hover:bg-green-300 transition duration-300"
        }`}
